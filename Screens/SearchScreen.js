...
export default class SearchScreen extends React.Component {
    ...
    fetchMoreTransaction = async () =>{
        ...
        if(enteredText[0].toUpperCase()==='B'){
            const transaction = await db.collection("transactions").where('bookId',"==",text).startAfter(this.state.latVisibleTransaction).limit(10).get()
            ...
            const transaction = await db.collection("transactions").where('studentId',"==",text).startAfter(this.state.latVisibleTransaction).limit(10).get()
            transaction.docs.map((doc)=>{
                this.setState({
                    allTransactions : [...this.state.allTransactions, doc.data()],
                    latVisibleTransaction: doc
