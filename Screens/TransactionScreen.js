...
import { Text, View, TouchableOpacity, TextInput, 
  Image, StyleSheet, KeyboardAvoidingView, ToastAndroid,
...
export default class TransactionScreen extends React.Component {
  ...
  checkStudentEligibilityForBookIssue = async()=>{
    ...
    if(studentRef.docs.length==0){
      ...
      Alert.alert("the student id does not exists in database")
    } else {
  ...
  handleTransaction = async()=>{
    ...
    if(!transactionType){
      Alert.alert(" the book doesn't exists in library");
      this.setState({
        scannedBookId : "",
        scannedStudentId: ""
      });
